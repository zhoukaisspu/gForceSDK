...
char* OYMXMLDocument::Identify( char* p, XMLNode** node )
{
...
bool OYMXMLDocument::Accept( XMLVisitor* visitor ) const
{
...
XMLNode::XMLNode( OYMXMLDocument* doc ) :
    document( doc ),
    parent( 0 ),
...
XMLNode* XMLText::ShallowClone( OYMXMLDocument* doc ) const
{
    if ( !doc ) {
        doc = document;
    }
...
XMLComment::XMLComment( OYMXMLDocument* doc ) : XMLNode( doc )
{
}
...
XMLNode* XMLComment::ShallowClone( OYMXMLDocument* doc ) const
{
    if ( !doc ) {
        doc = document;
    }
...
XMLDeclaration::XMLDeclaration( OYMXMLDocument* doc ) : XMLNode( doc )
{
}
...
XMLNode* XMLDeclaration::ShallowClone( OYMXMLDocument* doc ) const
{
    if ( !doc ) {
        doc = document;
    }
...
XMLUnknown::XMLUnknown( OYMXMLDocument* doc ) : XMLNode( doc )
{
}
...
XMLNode* XMLUnknown::ShallowClone( OYMXMLDocument* doc ) const
{
    if ( !doc ) {
        doc = document;
    }
...
// --------- XMLElement ---------- //
XMLElement::XMLElement( OYMXMLDocument* doc ) : XMLNode( doc ),
    closingType( 0 ),
    rootAttribute( 0 )
{
}
...
XMLNode* XMLElement::ShallowClone( OYMXMLDocument* doc ) const
{
    if ( !doc ) {
        doc = document;
    }
...
// --------- OYMXMLDocument ----------- //
OYMXMLDocument::OYMXMLDocument( bool _processEntities ) :
    XMLNode( 0 ),
    writeBOM( false ),
...
void OYMXMLDocument::InitDocument()
{
...
XMLElement* OYMXMLDocument::NewElement( const char* name )
{
...
XMLComment* OYMXMLDocument::NewComment( const char* str )
{
...
XMLText* OYMXMLDocument::NewText( const char* str )
{
    XMLText* text = new (textPool.Alloc()) XMLText( this );
...
XMLDeclaration* OYMXMLDocument::NewDeclaration( const char* str )
{
...
XMLUnknown* OYMXMLDocument::NewUnknown( const char* str )
{
...
int OYMXMLDocument::LoadFile( const char* filename )
{
    DeleteChildren();
    InitDocument();
...
int OYMXMLDocument::LoadFile( FILE* fp )
{
    DeleteChildren();
    InitDocument();
...
int OYMXMLDocument::SaveFile( const char* filename )
{
#if defined(_MSC_VER)
#pragma warning ( push )
#pragma warning ( disable : 4996 )        // Fail to see a compelling reason why this should be deprecated.
#endif
	//int fd = 11;//open(filename, O_RDWR|O_CREAT, 0644);
	FILE* fp;
	fopen_s(&fp, (const char*)filename, "wt+");
...
int OYMXMLDocument::SaveFile( FILE* fp )
{
...
int OYMXMLDocument::Parse( const char* p )
{
    DeleteChildren();
    InitDocument();
...
void OYMXMLDocument::Print( XMLPrinter* streamer )
{
...
void OYMXMLDocument::SetError( int error, const char* str1, const char* str2 )
{
    errorID = error;
    errorStr1 = str1;
    errorStr2 = str2;
...
void OYMXMLDocument::PrintError() const
{
    if ( errorID ) {
        ...
        printf( "OYMXMLDocument error id=%d str1=%s str2=%s\n",
                errorID, buf1, buf2 );
    }
}
...
bool XMLPrinter::VisitEnter( const OYMXMLDocument& doc )
{
